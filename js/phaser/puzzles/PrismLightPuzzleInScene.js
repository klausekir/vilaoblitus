 ...
 * O jogador interage com os elementos diretamente no cenário
 */
class PrismLightPuzzleInScene {
    ...
    create() {
        ...
        this.createPrisms(bgX, bgY, bgWidth, bgHeight);
        ...
        console.log('[PrismInScene] Puzzle criado com', this.prisms.length, 'prismas');
    }
    ...
    createPrisms(bgX, bgY, bgWidth, bgHeight) {
        ...
        slots.forEach((slotConfig, index) => {
            ...
            // Dados do slot
            const slot = {
                id: slotConfig.id || `prism_${index}`,
                x: x,
                y: y,
                requiredItem: slotConfig.requiredItem,
                hasItem: hasItem,
                rotation: slotConfig.rotation || 0,
                flipX: slotConfig.flipX || false,
                graphics: null,
                hitArea: null
            };
            ...
            if (hasItem) {
                ...
                const hitArea = this.scene.add.circle(x, y, 40, 0x000000, 0);
                ...
                // Círculo indicando o slot
                const slotCircle = this.scene.add.circle(x, y, 35, 0x444444, 0.3);
                slotCircle.setStrokeStyle(2, 0xff4444);
    ...
    updateRay(bgX, bgY, bgWidth, bgHeight) {
        ...
        for (let bounce = 0; bounce < maxBounces; bounce++) {
            ...
            this.prisms.forEach(slot => {
                if (!slot.hasItem || !slot.transformedVertices) return;
        ...
        // Re-desenhar para glow (seria melhor usar shader mas isso funciona)
    ...
    checkInventoryForItem(itemId) {
        // Verificar no inventoryManager
        if (window.inventoryManager?.hasItem) {
            return window.inventoryManager.hasItem(itemId);
        }
        // Verificar no gameStateManager
        if (window.gameStateManager?.getInventoryItem) {
            const item = window.gameStateManager.getInventoryItem(itemId);
    ...
    onSolved() {
        ...
        if (window.uiManager?.showNotification) {
            window.uiManager.showNotification('✅ Enigma resolvido!');
        }
    ...
    dropReward() {
        ...
        const dropPosition = { x: 50, y: 70 }; // Centro da tela, abaixo
        ...
        switch (action.type) {
            ...
            case 'changeBackground':
                ...
                if (action.newBackground && this.scene.background) {
                    // Carregar nova textura e trocar
